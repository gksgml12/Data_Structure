...
T_NODE* delete_bst(T_NODE* root, int data, bool* success){
	...
	if(data < (root->data)){
		...
		root->right = delete_bst(root->right,data,success);
			...
			deleted_node = root;
			if(root->left == NULL){// 1. both left and right are NULL
				new_root = root->right;
				free(deleted_node);
				*success = true;
				return new_root;
			} else {
				if(root->right ==NULL){ // 2. only right is NULL
					new_root = root->left;
					free(deleted_node);
					*success = true;
					return new_root;
				} else { // 3. both are not NULL
					search = root->left;
					while(search->right !=NULL) // most right of left ->left  understand??
						search = search->right;

					root->data = search->data;
					root->left = delete_bst(root->left, search->data, success); // from the root->left so root is not deleted
